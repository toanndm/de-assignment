...
**Cách A — Qua CLI (nhanh hơn):**

```bash
for /f "tokens=*" %i in ('docker compose ps -q airflow-webserver') do ^
docker exec -it %i airflow connections add postgres_assessment ^
  --conn-type postgres ^
  --conn-host postgres ^
  --conn-schema assessment ^
  --conn-login de ^
  --conn-password de ^
  --conn-port 5432
```
...
| --- | --- | --- |
| Airflow UI không lên | `airflow-init` chưa xong | Chờ thêm, xem `docker compose logs airflow-init` |
| DAG không xuất hiện | `pipeline.py` lỗi cú pháp | Xem `docker compose logs airflow-scheduler` |
| `NoBrokersAvailable` | Kafka chưa healthy | Chờ 30s, kiểm tra `docker compose ps kafka` |
| `Connection refused` port 5432 | Postgres chưa healthy | Kiểm tra `docker compose ps postgres` |
| `postgres_assessment` not found | Chưa tạo connection | Làm lại Bước 3 |
| `psycopg2` import error | Chưa activate venv | Chạy `.venv\Scripts\activate` trước |
